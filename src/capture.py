...
class Capture(QtCore.QThread):
    def __init__(self, deviceid, width=640, height=360):
        ...
        self.count = 0
        self.img_seq = sorted(glob.glob(os.path.join(self.deviceid, '*.jpg')))
    ...
    def get_cv_frame(self):
        ...
        :return: None
        """
        print self.count, len(self.img_seq)
        if self.count < len(self.img_seq):
            self.frame = cv2.imread(self.img_seq[self.count])
